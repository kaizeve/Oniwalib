...
[![tests](https://img.shields.io/badge/tests-735%2F735%20passing-2ea44f?style=flat-square)](#tests)
...
`@newsletter` / status / broadcast / lid / bot) · `e2e-message` 42 · `crypto` 19 (node/rts adapter parity —
...
`messages` 59 (incoming `pkmsg` → `messages.upsert` → `sendText`/`sendMessage`
